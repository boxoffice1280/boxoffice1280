#!/usr/bin/env bash
# OOAK inner driver — joint filter loop for the one-of-a-kind (genre
# singleton) template. Mirrors run_joint_filtered_probe.sh:
#
#   1. generate_one_of_a_kind_v2.py → over-sampled candidate seeds
...
K_MATES="${K_MATES:-1}"
DUPLICATE_PARTITION="${DUPLICATE_PARTITION:-5,4}"
...
for seed in "${SEEDS[@]}"; do
  full_input="${RUN_ROOT}/inputs/ooak_v2_s${seed}_full.jsonl"
  eval_input="${RUN_ROOT}/inputs/ooak_v2_s${seed}_eval.jsonl"
  "${PYTHON_BIN}" "${SCRIPT_DIR}/generate_one_of_a_kind_v2.py" \
    ...
    --k-mates "${K_MATES}" \
    --duplicate-partition "${DUPLICATE_PARTITION}" \
    --prompt-variant "${PROMPT_VARIANT}" \
    --out "${full_input}" \
    --manifest-out "${RUN_ROOT}/inputs/ooak_v2_s${seed}.manifest.json" \
...
PY
  "${PYTHON_BIN}" "${SHARED_V2_DIR}/build_question_only_inputs.py" \
    ...
    "${RUN_ROOT}/qonly_inputs/ooak_v2_s${seed}_full.jsonl" \
    "${RUN_ROOT}/qonly_inputs/ooak_v2_s${seed}_eval.jsonl"
done
...
for model_label in "${MODELS[@]}"; do
  ...
  for seed in "${SEEDS[@]}"; do
    launch_eval "${model_label}" \
      "${RUN_ROOT}/inputs/ooak_v2_s${seed}_eval.jsonl" \
...
for seed in "${SEEDS[@]}"; do
  ...
  "${PYTHON_BIN}" "${SHARED_V2_DIR}/build_joint_filtered_dataset.py" \
    --source-full "${RUN_ROOT}/inputs/ooak_v2_s${seed}_full.jsonl" \
    --source-eval "${RUN_ROOT}/inputs/ooak_v2_s${seed}_eval.jsonl" \
    --source-manifest "${RUN_ROOT}/inputs/ooak_v2_s${seed}.manifest.json" \
    --models "${MODELS_CSV}" \
    "${baseline_args[@]}" \
    "${qonly_args[@]}" \
    --keep-per-cell "${KEEP_PER_CELL}" \
    --out-full "${RUN_ROOT}/shared/ooak_v2_joint_s${seed}_full.jsonl" \
    --out-eval "${RUN_ROOT}/shared/ooak_v2_joint_s${seed}_eval.jsonl" \
    --out-manifest "${RUN_ROOT}/shared/ooak_v2_joint_s${seed}.manifest.json" \
