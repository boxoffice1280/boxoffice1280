#!/usr/bin/env python3
"""Balance PJ warmup so every eval chunk has >=2 fresh and >=2 mated variants (M=4).

Mirror of build_ooak_v4_balanced.py for the pivot-join template: direction
`<` = fresh prefix variant (no director-mate among the 9 predecessors),
`>` = mated (exactly one same-director mate). Every appended warmup row is
itself a valid PJ query (the named pivot has exactly one sharer).
"""
...
def prefix_sets_by_direction(
    ...
    for row in rows:
        ...
        directors = row["metadata"]["director_labels"]
        for idx, eid in enumerate(ctx):
            ...
            if not prefix:
                continue
            mated = any(str(directors[p]) == str(directors[eid]) for p in prefix)
            out[eid][">" if mated else "<"].add(prefix)
    return out
...
def main() -> None:
    ...
    parser.add_argument("--source-tag", type=str, default="pj_filtered_v2_reference")
    ...
    prompt_variant = str(source_full[0].get("metadata", {}).get("prompt_variant") or DEFAULT_PJ_PROMPT_VARIANT)
    dataset_variant = "pj_filtered_v4_balanced_m4"
        ...
        for direction in ("<", ">"):
            for variant_index in range(plan[direction]):
                ...
                for attempt in range(300):
                    ...
                    if direction == "<":
                        built = build_pj_fresh_prefix(target=target, pool=all_films, rng=rng)
                    else:
                        built = build_pj_mated_prefix(target=target, pool=all_films, rng=rng, k_mates=args.k_mates)
                    if built is None:
                        continue
                    prefix, pivot = built
                    tup = tuple(f.entity_id for f in prefix)
                    if tup in existing[target_id][direction]:
                        continue
                    built_prefix, built_pivot = prefix, pivot
                    break
                if built_prefix is None:
                    raise RuntimeError(f"Could not build {direction} variant {variant_index} for {target_id}")
                context = built_prefix + [target]
                rebuilt = make_pj_row(
                    ...
                    stage=f"warmup_pj_v4_balanced_m4_{direction}_{variant_index:02d}",
                    scheduled_cell=None,
                ...
                md["balanced_variant_prefix_entity_ids"] = [f.entity_id for f in built_prefix]
                md["balanced_variant_prefix_size"] = len(built_prefix)
                md["balanced_variant_builder"] = "build_pj_v4_balanced"
                added_rows.append(rebuilt)
                existing[target_id][direction].add(tuple(f.entity_id for f in built_prefix))
                update_first_seen_pj(rebuilt, first_seen)
    ...
    for idx, row in enumerate(full_rows):
        qid = f"pj_query_{idx:04d}"
        row["query_id"] = qid
        row["trace_id"] = qid
        row["longbench_id"] = f"pivot_join_v1_{idx:04d}"
